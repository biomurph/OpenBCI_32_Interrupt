 ...
 * and follow the directions to install chipKIT board files into the Arduino IDE.
 *    >>>>   WORKS WITH ARDUINO 1.6.5 ONLY    <<<<
...
volatile boolean ADS_DataAvailable = false;
...
void loop() {
  ...
  if(is_running){
    ...
    if(ADS_DataAvailable){   // this is set in the ISR
      ADS_DataAvailable = false;  // this needs to be reset, or else it will constantly flag us
...
char getChannelNumber(char n){  // this zero indexes the channel number
  if(n > '0' && n < '9'){
    n -= '1';
  }
  switch(n){
